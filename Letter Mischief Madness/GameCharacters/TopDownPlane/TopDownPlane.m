...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context{
    ...
    CGVector originalVelocity = self.sprite.physicsBody.velocity;
    
    CGVector zeroXVelocity = CGVectorMake(0.00, originalVelocity.dy);
    ...
    if(context == IsOutsideBoundaryContext){
        ...
        if([keyPath isEqualToString:@"isOutsideLeftBoundary"]){
            ...
            [self.sprite.physicsBody setVelocity:zeroXVelocity];
...
-(void)tapRight{
    ...
    if(self.isOutsideRightBoundary){
        return;
    }
